...
def normalize_search_response(
    ...
    """Convert a pageType=1 API response into the search tool output."""
...
def normalize_policy_detail(payload: Mapping[str, Any]) -> dict[str, Any]:
    """Convert a pageType=2 API response into grouped policy detail sections."""
...
def extract_count(payload: Mapping[str, Any], items: Iterable[Mapping[str, Any]]) -> int:
    for key in ("totalCount", "totalCnt", "count", "resultCnt", "totCnt"):
